...
def extract_dates_from_files():
    ...
    if not pmfs.empty:
        ...
        # Agregar: para cada data de tramite, marcar presença de tramite e computar média de ciclo_corte
        if '__tramite_date' in pmfs.columns:
            tramite_group = pmfs.dropna(subset=['__tramite_date'])
            for date_val, grp in tramite_group.groupby('__tramite_date'):
                entry = date_map.setdefault(date_val, {})
                entry.setdefault('data_tramite_status', date_val)
                # média de ciclo_corte para esse dia (somente registros com valor numérico)
                ciclo_vals = grp['__ciclo_num'].dropna()
                if not ciclo_vals.empty:
                    entry['ciclo_corte'] = float(ciclo_vals.mean())

        # Agregar últimas atualizações por data
        if '__ultima_date' in pmfs.columns:
            ultima_group = pmfs.dropna(subset=['__ultima_date'])
            for date_val, grp in ultima_group.groupby('__ultima_date'):
                entry = date_map.setdefault(date_val, {})
                entry.setdefault('ultima_atualizacao_relatorio_status', date_val)

        # fallback: se houver ciclo_corte sem data de tramite, tentar mapear pela data de emissão
        ciclo_without_tramite = pmfs[pmfs['__ciclo_num'].notna() & pmfs['__tramite_date'].isna()]
        if not ciclo_without_tramite.empty:
            for date_val, grp in ciclo_without_tramite.groupby('__emissao_date'):
                if pd.isna(date_val):
                    continue
                entry = date_map.setdefault(date_val, {})
                if entry.get('ciclo_corte') is None:
                    ciclo_vals = grp['__ciclo_num'].dropna()
                    if not ciclo_vals.empty:
                        entry['ciclo_corte'] = float(ciclo_vals.mean())
